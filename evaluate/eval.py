...
def evaluate_knn(original, generated, target_column, classification=True):
  train_data = generated.drop([target_column], axis=1).to_numpy()
  train_values = generated[target_column].to_numpy()
  test_data = original.drop([target_column],axis=1).to_numpy()
  test_values = original[target_column].to_numpy()
...
def create_statistics(data, target_column, model, categorical_columns = [], k = 100, classification = False, sample_size = 10000, method='randomforest'):
